/* =========================================
   main.js — bubbles, modal, audio (info bubble added)
   ========================================= */
...
(() => {
  ...
  /* ---------- Config (physics kept as-is from your last good build) ---------- */
  const PHYS = {
    SIZE_MIN: 160,
    SIZE_MAX: 260,
    SPEED_MIN: 12,
  ...
  /* ---------- Data ---------- */
  const INSTRUMENTS = [
    // NEW: informational bubble (no audio, text-only modal)
    {
      id: "about",
      name: "About Nada Nusantara",
      isInfo: true,
      htmlDesc: `
        <h3>🌊 About Nada Nusantara</h3>
        <p><strong>Nada Nusantara</strong> (Melodies of the Archipelago) is an interactive sound experience that brings Indonesia’s traditional instruments into a digital space. It invites visitors to listen, touch, and explore the rhythm of culture through movement and sound.</p>

        <h3>🎵 How It Works</h3>
        <p>Each floating bubble represents an instrument. By clicking, dragging, or adjusting the liquid-filled sliders, visitors can create layers of sound that blend together naturally. The bubbles move, collide, and respond to one another, reflecting the harmony and diversity found across Indonesia’s musical traditions.</p>

        <h3>💭 The Idea Behind It</h3>
        <p>This project reimagines cultural instruments for a new generation. It transforms sound into an experience that feels both modern and rooted in tradition. Nada Nusantara celebrates the living rhythm of Indonesian music, showing how culture can evolve while keeping its spirit alive.</p>
      `,
      tracks: [],
    },
  ...
  /* ---------- Simple helpers used below (animation code unchanged) ---------- */
  ...
  /* ---------- Bubbles (reuse your previous implementation) ---------- */
  ...
  function openModal(id) {
    ...
    if (data.isInfo) {
      // Info: title + rich HTML content, hide image/meta, no audio
      modalTitle.textContent = data.name;
      modalMedia.hidden = true;
      modalMeta.hidden = true;
      modalDesc.innerHTML = data.htmlDesc || "";
      player.pause();
      player.src = "";
    } else {
      // Instrument
  ...
  function closeModal() {
    ...
    modal.hidden = true;
    // no audio fade needed for info; for instruments:
